...
namespace TableAnalizer
{
    ...
    {   
        ...
        private async void AnalyzeTableData()
        {
            // Obtén los datos de la tabla
            var dataTable = dataGridView1.DataSource as DataTable;
            if (dataTable == null)
            {
                MessageBox.Show("No hay datos en la tabla para analizar.");
                return;
            }

            // Convierte los datos de la tabla a un string
            ...
            string tableData = dataStringBuilder.ToString();

            // Llama al método de OpenAIClient para analizar los datos
            string result = await OpenAIClient.AnalyzeData(tableData);
        ...
        public class OpenAIClient
        {
            ...
            private static readonly string apiKey = "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";
                    ...
                    using (var content = new StringContent(JsonConvert.SerializeObject(requestBody), Encoding.UTF8, "application/json"))
                    {
                        ...
                        try
                        {
                            ...
                            if (response.IsSuccessStatusCode)
                            {
                                ...
                                Console.WriteLine(responseContent); // Para ver la respuesta en la consola
        ...
        {
            AnalyzeTableData();
            string inputData = "write a haiku about AI";
            string result = await OpenAIClient.AnalyzeData(inputData);
            MessageBox.Show(result);
        }
    }
}